...
def stemfitter(x0,y0,aiamgn):
	...
	# flux = 0.0
	upline1 = np.arange(0,160,1) - 2
	upline2 = np.arange(0,160,1) 
	upline3 = np.arange(0,160,1) + 2
	# upliney = np.zeros_like(uplinex)
	# print(x0,y0)
	avg = 0.0
	alpha2 = np.arctan2(x0-900, y0-2048)
	# print(alpha2)
	rng = np.arange(70,110,0.5)
	for uplinex in (upline1,upline2,upline3):
		...
		for angle in rng:
			...
			newx = x0 + (uplinex)*cosalpha
			newy = y0 - (uplinex)*sinalpha
			# plt.plot(newx,newy)
...
def plumetrace(x0,y0,beta1,aiamgn):
	...
	# plt.plot(newx,newy)
	while ((latest > -2) and (xstep[-1] > 0) and (xstep[-1] < 1799)):
		# xm = x0 + step
		xkm = int(x0 + step*cosalpha1)
		ykm = int(y0 - step*sinalpha1)
		# print(xkm,ykm) 
...
# Triple File Prep
	...
	# x0,y0 =85,545
		...
		aiamgn += mgncv.mgn(np.float32(aiad),truncate = 3)
	...
	aiamgn = cv.medianBlur(np.float32(aiamgn),5)
	...
	aiamgn[aiamgn > 1] = 1
	displaymgn = copy.deepcopy(aiamgn)
...
#	df2 = df[df.rads <= 0]
	...
	try:
		...
		print("Error with map values - Exiting")
		return
	# break1=break2
	# plt.imshow(aiamgn)
	# break1=break4
		...
		if len(xstep) > 25:
			...
			if aiamgn[endcord] == -2:
				...
				for leng in range(8):
					...
					xw1 = xstep[xmid] + (width)*cos2
					yw1 = ystep[ymid] - (width)*sin2
	...
	#fig,ax = plt.subplots(facecolor='black')
	#plt.imshow(displaymgn,cmap = 'sdoaia171')
	#for x in range(len(loops)):
		# fig.plot_coord(loops[x][1],loops[x][0],linewidth = 3, color = 'white')
	#	plt.plot(loops[x][0],loops[x][1],linewidth = 3, color = 'white')
